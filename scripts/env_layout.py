"""The one and only layout for .env, and for the .env.example that documents it.

Both files are rendered from this module, so there is a single structure rather than
a hand-written template that drifts from what the app actually writes. A fresh
install's ``.env`` is therefore already sectioned and commented - it is not a flat
wall of keys that only becomes readable if somebody happens to copy the example
over it.
...
EXAMPLE_HEADER = """\
Audiobook Organizer configuration - example.

A copy of the layout the app generates, with every credential blank. It is here so
the settings can be read on the web without cloning, and so a fresh checkout has a
template to copy to .env. You do not have to: the app writes its own .env, with
these same comments, the first time it starts.

This file is committed. It must never contain a real key.

Generated by scripts/env_layout.py - run `python -m scripts.env_layout` to refresh.
"""

# Any key matching these is a secret: blanked in the example, and never given a
# built-in default anywhere.
...
SECTIONS: List[Tuple[str, str, List[str]]] = [
    ...
    ('Cache and logging', '',
     ['AO_CACHE_DB', 'AO_CACHE_MISS_TTL', 'AO_RESUME_SCANS', 'AO_LOG_LEVEL',
      'AO_THREADS']),

    ('Interface', '',
     ['AO_UI_DENSITY', 'AO_UI_ICON_SIZE', 'AO_UI_TOOLBAR_LABELS',
      'AO_UI_SHOW_COVERS', 'AO_UI_STATUS_STRIPE', 'AO_UI_ROW_TINT',
      'AO_UI_CONFIDENCE_COLOR', 'AO_UI_COLOR_BY_SOURCE', 'AO_UI_SHOW_FILTERS',
        ...
        for key in keys:
            ...
            if match:
                ...
                if comment := _key_comment(key):
                    out.append('')
                    out.extend(comment)
            value = '' if (blank_credentials and is_credential(key)) \
                else values.get(key, '')
...
def render_example() -> str:
    """The committed .env.example: built-in defaults, every credential blank."""
    from .settings import DEFAULT_PROVIDERS, PROVIDER_FIELDS, SCHEMA

    values = {key: default for key, (default, _kind, _help) in SCHEMA.items()}
    values['AO_PROVIDERS'] = ','.join(DEFAULT_PROVIDERS)
    for name, fields in DEFAULT_PROVIDERS.items():
        for field in PROVIDER_FIELDS:
            if field in fields:
                values[f'AO_PROVIDER_{name.upper()}_{field}'] = fields[field]
    # Machine-written window state is not a setting anyone seeds by hand.
    for key in ('AO_UI_WINDOW', 'AO_UI_COLUMN_WIDTHS', 'AO_UI_HIDDEN_COLUMNS',
                'AO_UI_GRID_WINDOW', 'AO_UI_GRID_COLUMNS', 'AO_UI_COPY_RECENT_LIST'):
        values.pop(key, None)
    return render(values, header=EXAMPLE_HEADER, blank_credentials=True,
