...
- **`AO_AUTHOR_INITIAL_STYLE`** - controls consecutive author initials. `compact`, the
  ...
  initial elsewhere in a name always receives its period, and a surname run onto the
  last initial is given its space back (`J.R.R.Tolkien` → `J.R.R. Tolkien`). This is
  applied to the value, not merely warned about, so `J. R. R. Tolkien`,
  `J R R Tolkien` and `J.R.R.Tolkien` all file under one author. Changing it re-cleans
  the books already in the list when Settings saves, so the table and Preview follow
  immediately.
